...
- It doesn't require any division or multiplication instructions.  It's literally shift and subtract.   The Knuth algorithm finds an estimated quotient digit using two-digit division, and then corrects it using two-digit multiplication, and then it does multiprecision combined multiplication and subtraction, where the multiplication is a multiprecision number by a single digit, and it does a possible corrective multiprecision addition if its estimated quotient turns out to be one too large.  
...
For the most part floating point arithmetic is implemented on top of the integer arithmetic already discussed.   There is necessarily some additional overhead in handling special values such as  `NaN` and `infinity`.   Additionally in some cases, the floating point calculations take longer than their equivalent integer calculations, because they have to be calculated to their full precision.  For example, integer division stops when it finds the full integer quotient.   Floating point division, on the other hand, must continue calculating until the quotient precision is filled and beyond to get proper rounding of the least significant digit.  The need to keep floating point numbers normalized also slows them down, which is unfortunate, but necessary.

There is also a design decision that I'm paying for now, and I'll have to fix it, though the consequences are spread throughout the entire floating point code. I bone-headedly decided to put the sign bit in the significand.  This has several consequences.  Not only does it mean that my 64-bit precision type actually has one bit less precision than `Float80` and makes thinking through exponent updates due to shifted significands more difficult, it also requires me to frequently make a copy of the significand to work with its magnitude, even though it's already in signed magnitude.  I have to do this because I need to set the sign bit to 0 to actually do the arithmetic, and then fix up the sign afterwards, and I can't alter the operand itself.  If the sign bit were in the exponent instead, I could eliminate at least half of the significand copies I make at the beginning of each operation, because I could just work with the operand's significand as-is.  I originally thought that putting the sign in the exponent would make working with the exponent more difficult, and it will, a little, but exponent updates are fast, even when they are complicated.  By comparison, copying the significand is expensive.  It's a pervasive change, and it will temporarily break everything related to `WideFloat`, but it's a change that must be done.
